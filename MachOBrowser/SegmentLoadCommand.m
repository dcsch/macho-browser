  ...
  if (self) {
    if (self.command == LC_SEGMENT) {
      struct segment_command *c =
          (struct segment_command *)(_data.bytes + _offset);
      char buf[17];
      ...
      for (NSUInteger i = 0; i < _nsects; ++i) {
        ...
        if (size > 0) {
          ...
          if (self.swapBytes)
            range =
                NSMakeRange(CFSwapInt32(sect->offset), CFSwapInt32(sect->size));
          else
      ...
      struct segment_command_64 *c =
          (struct segment_command_64 *)(_data.bytes + _offset);
      char buf[17];
      ...
      for (NSUInteger i = 0; i < _nsects; ++i) {
        ...
        if (size > 0) {
          ...
          if (self.swapBytes)
            range =
                NSMakeRange(CFSwapInt32(sect->offset), CFSwapInt64(sect->size));
          else
            range = NSMakeRange(sect->offset, sect->size);
          sectionData = [_data subdataWithRange:range];
        }
...
- (NSDictionary *)dictionary {
  ...
  if (cmd == LC_SEGMENT || cmd == LC_SEGMENT_64) {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:self.segName, @"segname", @(_vmaddr),
                                     @"vmaddr", @(_vmsize), @"vmsize",
                                     @(_fileoff), @"fileoff", @(_filesize),
                                     @"filesize", @(_maxprot), @"maxprot",
                                     @(_initprot), @"initprot", @(_nsects),
                                     @"nsects", @(_flags), @"flags", nil, nil];
  }
