...
- (void)makeWindowControllers {
  MachOWindowController *controller =
      [[MachOWindowController alloc] initWithWindowNibName:@"MachOWindow"];
...
- (BOOL)readFromFileWrapper:(NSFileWrapper *)fileWrapper
  ...
  if (fileWrapper.directory) {
    ...
    NSDictionary<NSString *, id> *infoDict = [NSPropertyListSerialization
        propertyListWithData:infoFileWrapper.regularFileContents
    ...
    if (execFileWrapper)
      return [self readFromData:execFileWrapper.regularFileContents
                         ofType:typeName
                          error:outError];
    else
      return NO;
